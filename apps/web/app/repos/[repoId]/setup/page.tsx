...
const MCP_JSON = JSON.stringify(
  { mcpServers: { contextos: { type: "stdio", command: "contextos", args: ["mcp"] } } },
  null,
  2,
);
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="mt-1 text-sm text-[var(--muted)]">
        ...
        team&apos;s approved memory through the ContextOS MCP server — automatically, before it acts.
      </p>

      <Step n={1} title="Install the CLI">
        <CommandBlock command={`npm install -g ${PKG}`} />
        <p className="mt-2 text-xs text-[var(--muted)]">
          The package is <code>{PKG}</code>; the installed command is <code>contextos</code>.
        </p>
      </Step>

      <Step n={2} title="Authenticate">
        <CommandBlock command="contextos login" />
        <p className="mt-2 text-sm text-[var(--muted)]">
          Stores an API token in <code>~/.contextos/credentials.json</code>. Prefer to set it
      ...
      <Step n={3} title="Connect this repo">
        <CommandBlock command={`contextos init --repo ${repoId}`} />
        <p className="mt-3 text-sm text-[var(--muted)]">Run from the repo root. This generates:</p>
        <ul className="mt-2 space-y-1 text-sm text-[var(--muted)]">
          <li>
            <code>.contextos/config.json</code> — links this directory to the repo
          </li>
          <li>
            <code>CLAUDE.md</code> — guidance the agent reads
          </li>
          <li>
            <code>.mcp.json</code> — registers the ContextOS MCP server
          </li>
          <li>
      ...
      <Step n={4} title="Use it in Claude Code">
        <p className="text-sm text-[var(--muted)]">
          Open Claude Code in this repo. It discovers the <code>contextos</code> MCP server and can
          call:
        </p>
      ...
      <Card className="mt-8 p-6">
        ...
        <p className="mt-1 text-xs text-[var(--muted)]">
          Written by <code>contextos init</code>. If Claude Code can&apos;t launch the server, ensure{" "}
          <code>contextos</code> is on your PATH.
        </p>
        <div className="mt-3">
          <Code>{MCP_JSON}</Code>
        </div>
