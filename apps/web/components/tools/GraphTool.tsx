...
/** Force-directed graph of a project's repos, memories, and sessions. */
export function GraphTool({ workspaceId }: { workspaceId: string }) {
  const router = useRouter();

  const { data: graph } = useQuery({
    queryKey: ["graph", workspaceId],
    queryFn: () => api<GraphData>(`/workspaces/${workspaceId}/graph`),
    enabled: !!workspaceId,
  });
