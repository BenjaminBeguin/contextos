...
import { Button, Card, Code, Input, Spinner, Textarea } from "./ui";
...
/**
 * The complete settings surface for a single repo — connect command, codebase
 * scan, PR reviewer (+ skills), repo context, sessions, danger zone. Rendered
 * both on the repo page and inside the project Setup drawer, so there is no
 * separate "full settings" page to bounce to.
 */
export function RepoSettings({ repoId, onDeleted }: { repoId: string; onDeleted?: () => void }) {
  ...
  const count = (status: string) => repo.memoryCounts.find((c) => c.status === status)?._count ?? 0;

  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-3">
        <Stat label="Approved" value={count("approved")} />
        <Stat label="Inbox" value={count("proposed")} />
      ...
      {repo.viewerRole === "owner" ? (
        <DangerZone repoId={repoId} fullName={repo.fullName} onDeleted={onDeleted} />
      ) : null}
    </div>
  );
}
    ...
    <Card className="p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--muted)]">
            Scan the repo&apos;s README, manifest, and structure to propose starter memories for
            review in the inbox. Uses this workspace&apos;s Anthropic key, or a heuristic fallback.
          </p>
        </div>
  ...
  return (
    <Card className="p-6">
      ...
      <p className="mt-1 text-xs text-[var(--muted)]">
        Reviews pull requests grounded in this repo&apos;s approved memories. Run it automatically in
        CI, or on demand below. Uses this workspace&apos;s Anthropic key.
      </p>
...
function ReviewerSkills({
  ...
  return (
    <div className="mt-6 border-t border-[var(--border)] pt-4">
      ...
      <p className="mt-1 text-xs text-[var(--muted)]">
        Reusable, named instruction sets shared across this project&apos;s repos.
      </p>
...
function RepoContextCard({ repo, repoId }: { repo: RepoDetail; repoId: string }) {
  ...
  const notConnected = resync.isError && (resync.error as Error).message === "github_not_connected";
    ...
    <Card className="p-6">
      ...
      {!sessions || sessions.length === 0 ? (
        <p className="mt-3 text-sm text-[var(--muted)]">
          No sessions yet. Claude Code records them via <code>record_session_summary</code>.
        </p>
      ) : (
